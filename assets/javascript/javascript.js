...
// Takes user input for desired topic and creates new button
function loadPage() {
    $('#buttons').empty();
    $('#giphys').empty();
...
// Protocol for finding and storing gifs from online source
function loadGiphys(topic) {
    ...
    xhr.done(function (data) {
        ...
        for (let j = 0; j < results.length; j++) {
            ...
            $('#giphys').append("<div class=img_" + j + "><img src='" + results[j]["images"]["original_still"]["url"] +
            "' class='" + j + "'alt='' data-state='static' src_static=' " + results[j]["images"]["original_still"]["url"] +
            "' src_non_static='" + results[j]["images"]["downsized"]["url"] + "'></div>");
